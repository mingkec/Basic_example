...
def train_op(system_setting,custom_setting):
    ...
    with tf.control_dependencies(update_ops):
        op=tf.train.AdamOptimizer(lr,epsilon=0.001).minimize(total_loss,global_step=global_step)
        return [op],custom_setting

def accuracy_func(system_setting,custom_setting):
    logits=system_setting['prediction'][0]
    label=system_setting['label']['y_label']
    
    accuracy_func=np.mean(np.equal(label.argmax(1),logits.argmax(1)))
    
    return accuracy_func,custom_setting
...
system_setting['model_name']='example'
